...
internal class BilibiliConnectionService : IBarrageConnectionProvider
{
    ...
    public async Task<bool> ConnectAsync(int roomId, Action<Result> onReceive, CancellationToken cancellation = default)
    {
        ...
        BroadCastServer connectedServer = null;
        foreach (var server in broadCastInfo.Servers)
        {
            try
            {
                _logger.LogInformation("Try connect bilibili live broadcast host:{Host} in port:{Port}", server.Host, server.Port);
                var client = new TcpClient();
                await client.ConnectAsync(server.Host, server.Port, cancellation);
                if (client.Connected)
                {
                    tcpClient = client;
            ...
            {
                _logger.LogWarning(ex, "Connect to {Host}:{Port} failed, try next server.", server.Host, server.Port);
            }
        }

        if (tcpClient == null || !tcpClient.Connected)
        {
            _logger.LogWarning("All bilibili live broadcast servers connect failed (network may be restricted), roomId:{RoomId}. Fallback to HTTP polling mode.", roomInfo.RoomId);
            await PollDanmuByHttpAsync(roomInfo.RoomId, onReceive, cancellation);
            return true;
        }

        _logger.LogInformation("Tcp connected bilibili live broadcast host:{Host} in port:{Port}", connectedServer!.Host, connectedServer.Port);
        ...
        try
        {
            ...
            // 探测 TCP 是否真的可用：8 秒内是否有数据到达。
            // 受限网络（如代理拦截）下 TCP 握手可能“成功”但数据被掐断，此时应回退 HTTP 轮询。
            var tcpUsable = false;
            var deadline = DateTime.UtcNow.AddSeconds(8);
            while (!cancellation.IsCancellationRequested && DateTime.UtcNow < deadline)
            {
            ...
            if (!tcpUsable)
            {
                _logger.LogWarning("Tcp connected but no data within 8s (network restricted), roomId:{RoomId}. Fallback to HTTP polling mode.", roomInfo.RoomId);
                try
                {
                    tcpClient.Close();
                }
    ...
    /// <summary>
    /// HTTP 轮询模式：长连接不可用时，定时拉取 gethistory 弹幕历史推给前端。
    /// </summary>
    private async Task PollDanmuByHttpAsync(int roomId, Action<Result> onReceive, CancellationToken cancellation)
    {
        _logger.LogInformation("Enter HTTP polling mode for room:{RoomId}", roomId);
        ...
        while (!cancellation.IsCancellationRequested)
        {
            try
            {
                var list = await _bilibiliApiService.GetDanmuHistoryAsync(roomId);
                // gethistory 最新在前，反转成时间正序，先推旧的再推新的
                list.Reverse();
                foreach (var info in list)
                {
                    var key = info.Mid + "|" + info.Time.Ticks + "|" + info.Comment;
                    if (seen.Add(key))
                    {
                        onReceive.Invoke(new Result(MessageType.DANMU_MSG, info));
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.LogWarning(ex, "Poll danmu history failed: {Message}", ex.GetBaseException().Message);
            }
